...
| Deleting a project, dataset, or volume removes its generated files but never registered sources | `projects/services.py` (`_plan_file_cleanup`, runs after commit); `DeleteRemovesGeneratedFilesTests` and `DeleteThroughTheApiTests` in `projects/tests.py` | May be described as implemented data-management behavior; not a storage-efficiency result |
