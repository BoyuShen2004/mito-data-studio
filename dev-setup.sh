...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage: ./dev-setup.sh [--install-deps] [--smoke] [--check-git]
...
CHECK_OUT="$(python "$REPO_ROOT/backend/manage.py" check 2>&1)" || {
  printf '%s\n' "$CHECK_OUT" >&2
  die "django check failed — fix: see errors above"
}
MIGRATE_OUT="$(python "$REPO_ROOT/backend/manage.py" migrate --noinput 2>&1)" || {
  printf '%s\n' "$MIGRATE_OUT" >&2
