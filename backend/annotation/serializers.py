...
class AnnotationSubmissionSerializer(serializers.ModelSerializer):
    ...
    label_comment_count = serializers.SerializerMethodField()

    def get_label_comment_count(self, obj) -> int:
        return obj.label_comments.count()
...
class ReviewLabelCommentSerializer(serializers.ModelSerializer):
    ...
    volume_name = serializers.CharField(
        source="task.volume.name", read_only=True, default=""
    )
    ...
    round_number = serializers.IntegerField(
        source="submission.round_number", read_only=True
    )
    submission_source = serializers.CharField(
        source="submission.source", read_only=True
    )
    submission_review_status = serializers.CharField(
        source="submission.review_status", read_only=True
    )

    class Meta:
        model = ReviewLabelComment
        # POST is an intentional upsert on (submission, label_id); the view
        # owns the author check and update path, so DRF's autogenerated unique
        # validator must not reject an edit before it reaches that logic.
        validators = []
        fields = [
            "id",
            "submission",
...
class AssignmentPlanTaskSerializer(serializers.ModelSerializer):
    ...
    dataset_id = serializers.IntegerField(
        source="volume.dataset_id", read_only=True, allow_null=True
    )
    dataset_name = serializers.CharField(
        source="volume.dataset.name", read_only=True, default="Ungrouped volumes"
    )
    file_format = serializers.CharField(source="volume.file_format", read_only=True)
    shape_z = serializers.IntegerField(source="volume.shape_z", read_only=True)
    shape_y = serializers.IntegerField(source="volume.shape_y", read_only=True)
    ...
    class Meta:
        model = AnnotationTask
        fields = [
            "id", "project", "volume", "volume_name", "dataset_id",
