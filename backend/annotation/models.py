...
class HardCase(models.Model):
    ...
    * **Anyone with the token URL** — the original public, no-account,
    ...
    # Why this instance was flagged. One reason, because a hard case is one
    # question. Blank is the default and is meaningful: every case recorded
    # before this field existed has none, and the list renders those as
    # "Uncategorised" rather than guessing on their behalf.
    category = models.CharField(
        max_length=20, choices=HardCaseCategory.choices, blank=True
    )
