...
class AuditVerb(models.TextChoices):
    ...
    HARD_CASE_CATEGORISED = "hard_case.categorised", "Hard case category set"
...
class HardCaseCategory(models.TextChoices):
    """Why an instance was flagged as a hard case.

    Deliberately a **single** choice, and deliberately about the *labelling*
    rather than the biology: a hard case is a question somebody needs answered,
    so it has one reason, and that reason is what the reader is being asked to
    act on.

    Blank is a real value and the default: every case recorded before this
    field existed has none, and forcing a guess onto them would invent data.
    """

    UNCERTAIN = "uncertain", "Uncertain — needs a second opinion"
