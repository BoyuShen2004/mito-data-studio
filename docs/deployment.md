...
Application-owned data uses one host-level policy: directories are `2775`,
regular files are `0664`, and the data tree has a default ACL equivalent to
`u::rwx,g::rwx,o::rx`. The setgid bit and the production units' `UMask=0002`
make new uploads, working labels, lock files, pyramids, and caches inherit the
same service group and access. Do not use `0777`: account, project, team, and
assignment rules remain the authorization boundary for writes through the API.
External registered source roots stay read-only to the service.
...
```text
public URL -> reverse proxy -> :18191 -> v1.1.5 checkout -> production DB -> production MITO_DATA_ROOT
```
