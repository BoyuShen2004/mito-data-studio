#!/usr/bin/env bash
#
# run.sh — start Mito Data Studio (Django + Vite).
#
#   make dev
#
# Assumes `make setup` has already been run. Always frees ports left by a
...
#
#   VITE_HOST=0.0.0.0 DJANGO_HOST=0.0.0.0 NO_BROWSER=1 make dev
#
set -euo pipefail
...
[[ -f "$REPO_ROOT/.env" ]] || die "No .env found. Run 'make setup' first."
[[ -d "$REPO_ROOT/frontend/node_modules" ]] || die "Frontend dependencies not installed. Run 'make setup' first."
...
$SETSID python "$REPO_ROOT/manage.py" runserver "${DJANGO_HOST}:${DJANGO_PORT}" \
